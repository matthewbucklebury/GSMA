...
function renderLeague() {
  ...
  renderTable($("#league-table"), [
    ...
    { key: "company", label: "Company", render: r =>
        `<a class="link" onclick="openCompany(${r.company_id})">${esc(r.company)}</a>` },
    ...
    { key: "guide_sum", label: "Guide country sum", num: true, render: r =>
        r.guide_sum ? `${fmt(r.guide_sum)} <span class="muted small">(${r.guide_countries})</span>` : "<span class='muted'>—</span>" },
...
function renderMnos() {
  ...
  $("#mno-count").textContent = `${rows.length} operators`;
  renderTable($("#mno-table"), [
    ...
    { key: "markets", label: "Active markets", num: true, sortVal: r => r.markets.length,
      render: r => fmt(r.markets.length) },
    { key: "marketlist", label: "Markets", sortVal: r => r.markets.length, render: r =>
        `<span class="small muted">${esc(r.markets.slice(0, 7).map(m => m.country).join(", "))}${r.markets.length > 7 ? ` +${r.markets.length - 7}` : ""}</span>` },
    ...
    { key: "owned", label: "Owns in", sortVal: r => Object.keys(r.owned).length, render: r =>
        `<span class="small muted">${esc(Object.keys(r.owned).slice(0, 6).join(", "))}</span>` },
    { key: "anchor_tenant_of", label: "Anchor tenant of", sortVal: r => r.anchor_tenant_of.length,
      render: r => `<span class="small muted">${esc(r.anchor_tenant_of.slice(0, 5).join(", "))}${r.anchor_tenant_of.length > 5 ? " +" + (r.anchor_tenant_of.length - 5) : ""}</span>` },
  ], rows, r => openCompany(r.company_id));
}
...
function renderCountries() {
  ...
  let rows = state.countries.filter(c =>
    ...
    (c.region || c.owners_count > 0 || Object.keys(c.stats).length > 0 || f));
  $("#country-count").textContent = `${rows.length} countries`;
  const st = (r, m) => r.stats[m] ? r.stats[m].value : null;
  renderTable($("#country-table"), [
    { key: "name", label: "Country", render: r => `<a class="link" onclick="openCountry(${r.id})">${esc(r.name)}</a>` },
    { key: "region", label: "Region", render: r => `<span class="pill">${esc(r.region || "")}</span>` },
    { key: "towers_total", label: "Total towers (est.)", num: true, sortVal: r => st(r, "towers_total"),
      render: r => `<b>${fmt(st(r, "towers_total"))}</b>` },
    { key: "holdings_sum", label: "Sum of tracked owners", num: true, render: r => fmt(r.holdings_sum) },
    { key: "towerco_share", label: "Towerco share", num: true,
      render: r => r.towerco_share !== null ? r.towerco_share + "%" : "—" },
    { key: "owners_count", label: "Owners tracked", num: true, render: r => fmt(r.owners_count) },
    ...
    { key: "sims", label: "SIMs/tower", num: true, sortVal: r => st(r, "sims_per_tower"),
      render: r => fmt(st(r, "sims_per_tower")) },
    { key: "asof", label: "As of", sortVal: r => r.stats.towers_total ? r.stats.towers_total.as_of : "",
      render: r => r.stats.towers_total ? esc(r.stats.towers_total.as_of) : "" },
...
function drawMap(metric, rows) {
  ...
  const scale = v => {
    ...
    const q = d3.bisectLeft(vals, v) / Math.max(vals.length - 1, 1); // quantile position
  ...
  svg.selectAll("path.country").data(countries).join("path")
    ...
    .attr("fill", d => { const r = byName[d.properties.name]; return r ? scale(r.value) : css("--surface-1"); })
...
/* ------------------------------------------------ explore & compare */
const CMP_VIEWS = {
  country: [["ownership", "Ownership breakdown (stacked)"], ["towers_total", "Total towers"],
            ["towerco_share", "Towerco share %"], ["sims_per_tower", "SIMs per tower"]],
  company: [["holdings", "Towers by country (stacked)"], ["league", "Global tower count (league)"]],
};
...
async function renderCompare() {
  ...
  if (kind === "country" && view === "ownership") {
    ...
    const val = c => view === "towerco_share"
      ? towercoShare(c) : (c.stats[view] ? c.stats[view].value : null);
...
function closeDrawer() { $("#drawer").classList.remove("open"); if (drawerChart) { drawerChart.destroy(); drawerChart = null; } }
...
async function openCountry(id) {
  ...
  const stats = d.observations.filter(o => !o.company);
  const latestStats = latestBy(stats, o => o.metric);
  const statOf = m => latestStats.find(s => s.metric === m);
  const body = $("#drawer-body");
  body.innerHTML = `
    <h2>${esc(c.name)} <span class="pill">${esc(c.region || "")}</span></h2>
    <div class="stat-row">
      ${tile(fmt(statOf("towers_total")?.value ?? statOf("towers_total")?.value_text), "est. total towers", statOf("towers_total"))}
      ${tile(fmt(latest.reduce((s, o) => s + (o.value || 0), 0)), "sum of tracked owners")}
      ${tile(fmt(statOf("sims_per_tower")?.value), "SIMs per tower", statOf("sims_per_tower"))}
      ${tile(esc(statOf("sim_penetration_pct")?.value_text ?? "—"), "SIM penetration", statOf("sim_penetration_pct"))}
    </div>
...
async function openCompany(id) {
  ...
  const body = $("#drawer-body");
  body.innerHTML = `
    <h2>${esc(c.name)} <span class="pill ${c.type}">${esc(c.type)}</span></h2>
    ${c.owners ? `<div class="muted small">Owners: ${esc(c.owners)}</div>` : ""}
    <div class="stat-row">
      ${league ? tile("#" + league.rank, "league rank") : ""}
      ${league ? tile(fmt(league.towers), "towers (league)", league) : ""}
    ...
    ${d.footprint.length ? `<h3>League footprint</h3><div class="small muted">${esc(d.footprint.join(", "))}</div>` : ""}
    ${d.mno_markets.length ? `<h3>Active as MNO in</h3><div class="small muted">${esc(d.mno_markets.join(", "))}</div>` : ""}
    ${d.anchor_tenants.length ? `<h3>Public anchor tenants</h3><div class="chips">${d.anchor_tenants.map(t =>
    ...
    ${d.anchor_tenant_of.length ? `<h3>Anchor tenant of</h3><div class="chips">${d.anchor_tenant_of.map(t =>
...
function historyTable(obs, withCountry) {
  return `<table class="data"><tr>${withCountry ? "<th>Country</th>" : "<th>Company</th>"}<th>Metric</th><th>Seg</th><th class="num">Value</th><th>As of</th><th>Conf.</th><th>Source</th><th>Note</th></tr>` +
    obs.map(o => `<tr><td>${esc(withCountry ? (o.country || "global") : (o.company || "—"))}</td>
...
function renderAbout() {
  ...
  $("#about-content").innerHTML = `
    ...
    <p>Every fact is an <i>observation</i> tagged with year+quarter (or unknown), source and
    confidence. The “View as of” selector replays the dataset at earlier periods; new data
    entered on the Data entry page is stored alongside, never replacing, the extracted history.</p>
